...
#
# There is no post-fs-data.sh here, and that is the point rather than an
# omission: the qualcomm module has one to mount debugfs, and on MediaTek
# nothing has to be mounted at all.
for script in "$here"/customize.sh "$here"/uninstall.sh; do
	if ! sh -n "$script"; then
		note "$(basename "$script") does not parse"
	fi
done

if [ -f "$here/post-fs-data.sh" ]; then
	note "post-fs-data.sh is back; pack.sh does not ship it and nothing needs it"
fi
