...
#
#   ./pack.sh [<path to MtkPdInfoRoot.apk>]
#
...
if [ -z "$apk" ]; then
	...
	apk=$(find "${ANDROID_BUILD_TOP:-$HOME/witaqua/16.2}/out/soong/.intermediates" \
		-name MtkPdInfoRoot.apk -print -quit 2>/dev/null || true)
fi

if [ ! -f "$apk" ]; then
	echo "pack.sh: no MtkPdInfoRoot.apk (pass one, or build it first)" >&2
	exit 1
fi
...
rm -rf "$out/stage" "$zip"
mkdir -p "$out/stage"
cp "$here"/module.prop "$here"/customize.sh "$here"/uninstall.sh "$out/stage/"
cp "$apk" "$out/stage/MtkPdInfoRoot.apk"
